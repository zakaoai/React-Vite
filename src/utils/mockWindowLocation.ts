/* eslint-disable no-implicit-globals  -- GIST */
/* eslint-disable @typescript-eslint/no-unsafe-type-assertion -- GIST */

/* eslint-disable @typescript-eslint/no-magic-numbers -- GIST */
/* eslint-disable @typescript-eslint/prefer-destructuring -- GIST */
/*
...
class WindowLocationMock implements Location {
  ...
  // eslint-disable-next-line @typescript-eslint/class-methods-use-this -- Do Nothing
  reload() {
    // Do nothing
  }
}
...
export class WindowHistoryMock implements History {
  ...
  public sessionHistory: [{ state: unknown; url: string }] = [{ state: null, url: window.location.href }]
  ...
  go(delta = 0) {
    ...
    if (newIndex < 0 || newIndex >= this.length) {
      ...
      this.index = newIndex
      ;(window.location as WindowLocationMock).internalSetURLFromHistory(this.sessionHistory[this.index].url)
